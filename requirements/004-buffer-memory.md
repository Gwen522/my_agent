...
| 技术 | 是什么 | 在这个需求里的作用 |
|------|--------|-------------------|
| **HumanMessage** | LangChain 消息对象，role=human | 不再传裸字符串，手动构造便于存数组 |
| **AIMessage** | LangChain 消息对象，role=ai | 模型回复也包装成对象，才能放进 history |
| **扩展开运算符 `...`** | JS 语法，把数组展开放到新数组里 | `[...this.history, msg]` = 把历史消息逐条插入 |
| **Buffer Memory** | 最常见记忆策略：全量存、全量拼 | 当前实现方式，代码越少越好理解 |
| **上下文窗口** | 模型能处理的最大 token 数 | 004 暂不管限制，历史太长时 005 处理 |
